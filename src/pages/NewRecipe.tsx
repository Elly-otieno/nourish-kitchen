...
export function NewRecipe() {
  ...
  const [ingredients, setIngredients] = useState<Ingredient[]>([
    { id: '1', qty: '250', unit: 'Grams', category: 'Main', name: 'Organic Flour' }
  ]);
  ...
  const publishRecipe = async () => {
    if (!title) {
      setNotification({ type: 'error', message: 'Please provide a recipe title.' });
      return;
    }
    ...
    try {
      ...
      if (heroImageFile) {
        ...
        result = await api.createRecipe(formData);
      } else {
        result = await api.createRecipe({
          ...recipeData,
          categories,
          ingredients,
          steps,
          isPublished: true,
          heroImage: heroImage || 'https://images.unsplash.com/photo-1495521821757-a1efb6729352?auto=format&fit=crop&q=80'
        } as any);
      }

      setNotification({ type: 'success', message: 'Recipe published successfully!' });
      setTimeout(() => {
        navigate(`/recipes/${result.id}`);
      }, 1500);
    } catch (error: any) {
      console.error('Failed to publish recipe:', error);
      setNotification({ type: 'error', message: error.message || 'Failed to publish recipe.' });
    } finally {
    ...
    >
      ...
      <div className="h-16 md:h-20 flex justify-between items-center px-4 md:px-12 border-b border-stone-100 bg-white/80 backdrop-blur-md sticky top-0 md:top-20 z-20">
        <div className="flex flex-col min-w-0 mr-4">
          <h2 className="font-serif text-lg md:text-xl font-medium text-primary opacity-90 truncate">New Kitchen Creation</h2>
        </div>
      ...
      <div className="mx-auto max-w-4xl py-10 md:py-16 px-6 md:px-12 space-y-12 md:space-y-20 mb-20 md:mb-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <ChevronDown size={16} className="absolute right-2 bottom-4 text-stone-300 pointer-events-none" />
                </div>
              </div>
            ...
            >
              {heroImage ? (
                ...
                <>
                  ...
                  <h3 className="font-serif text-xl md:text-2xl font-semibold text-on-background text-center">Click to upload your hero image</h3>
                  <p className="mt-2 text-center text-xs md:text-sm text-stone-400 font-medium md:max-w-xs">
                    High-resolution photography recommended for an editorial feel (JPEG, PNG up to 10MB)
                  </p>
                </>
              )}
                ...
                >
                  ...
                  <div className="w-full sm:col-span-2 space-y-1 relative">
                    ...
                    <div className="relative">
                      ...
                      <ChevronDown size={14} className="absolute right-2 bottom-3 text-stone-300 pointer-events-none" />
                    </div>
                    {ing.unit === 'Other' && (
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                        <input 
                          ...
                          onChange={(e) => updateIngredient(ing.id, 'customUnit', e.target.value)}
                  ...
                  <div className="w-full sm:col-span-3 space-y-1 relative">
                    <label className="sm:hidden text-[10px] text-stone-400 font-bold uppercase tracking-widest">Category</label>
                    <select 
                      value={ing.category}
                    ...
                    <ChevronDown size={14} className="absolute right-2 bottom-3 text-[#b58e3e] opacity-40 pointer-events-none" />
                  </div>
        ...
        <section className="space-y-8">
          <div className="flex items-center justify-between">
            <label className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#b58e3e]">Instructions</label>
            <button 
              onClick={addStep}
              className="flex items-center gap-2 text-xs font-bold text-primary hover:text-[#b58e3e] transition-colors py-2"
            >
      ...
      <footer className="mt-20 py-16 border-t border-stone-100 text-center bg-stone-50/30">
        <p className="font-serif italic text-stone-300 text-2xl">Nourish Kitchen</p>
        <p className="font-sans text-[10px] text-stone-200 mt-2 uppercase tracking-[0.3em]">Home Cooking for the Soul</p>
      </footer>
    </motion.div>
  );
}
