...
export function Archives() {
  ...
  useEffect(() => {
    console.log("LOG: Archive component mounted. Loading data...");
    loadArchives();
  }, []);
  ...
  const loadArchives = async () => {
    ...
    try {
      ...
      console.log(
        "LOG: Archive data received. Recipes:",
        data.recipes.length,
        "Blogs:",
        data.blogs.length,
      );
      ...
      console.error("ERROR: Failed to load archives:", error);
    } finally {
      setLoading(false);
      console.log("LOG: Archive loading sequence complete.");
    }
  };
  ...
  const handlePermanentDelete = async () => {
    ...
    console.log(`LOG: Initiating permanent delete for ${type} ID: ${id}`);
    setActionLoading(id);
    ...
    try {
      ...
      console.log(`LOG: Permanent delete successful for ${id}`);
      ...
      alert(
        `Could not permanently delete: ${error instanceof Error ? error.message : "Check permissions"}`,
      );
    } finally {
      setActionLoading(null);
      setConfirmDelete(null);
      console.log("LOG: UI state reset after permanent delete attempt.");
    }
  };

  const handleRestore = async (id: string, type: "recipe" | "blog") => {
    console.log(`LOG: Initiating restore for ${type} ID: ${id}`);
    setActionLoading(id);
    try {
      if (type === "recipe") {
      ...
      alert("Failed to restore item. It might have been permanently removed.");
    } finally {
      setActionLoading(null);
    }
  };
    ...
    >
      <div className="max-w-6xl mx-auto">
        <header className="mb-12">
          ...
          <h1 className="font-serif text-5xl md:text-6xl text-primary font-medium tracking-tight mb-4">
            The Faults
          </h1>
          <p className="font-sans text-stone-500 max-w-2xl text-lg">
            Manage your archived recipes and journal entries. Restore them to
            the main library or permanently purge them from the kitchen.
          </p>
        </header>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="font-sans text-stone-500 text-center mb-10 leading-relaxed">
                This action is irreversible. Are you absolutely certain you wish
                to purge this item from the kitchen vaults forever?
              </p>
