...
export function BlogDetail() {
  ...
  useEffect(() => {
    async function loadPost() {
      ...
      try {
        ...
        setPost(data);
        if (user) {
          setIsBookmarked(data.bookmarkedBy?.includes(user.id) || false);
        }
      } catch (error) {
  ...
  const toggleBookmark = async () => {
    ...
    try {
      ...
      setPost(updated);
      setIsBookmarked(updated.bookmarkedBy?.includes(user.id) || false);
    } catch (error) {
  ...
  const handleSoftDelete = async () => {
    ...
    try {
      ...
      setNotification({ type: 'success', message: 'Journal entry moved to archives.' });
      setTimeout(() => {
        setNotification(null);
        navigate('/blog/all');
      }, 2000);
    } catch (error) {
      console.error('Failed to move blog to archives:', error);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-serif italic text-emerald-950">Opening the archives...</p>
        </div>
      </div>
    );
  }
  ...
  if (!post) {
    return (
      <div className="py-32 text-center bg-stone-50 min-h-screen flex flex-col items-center justify-center">
        <h2 className="font-serif text-3xl font-bold text-emerald-950 mb-4">Article Not Found</h2>
        <button 
          onClick={() => navigate(-1)}
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 sm:p-0">
            ...
            >
              ...
              <p className="text-stone-500 text-sm mb-8 leading-relaxed">This journal entry will be removed from the public library but preserved in the kitchen vaults. You can restore it at any time.</p>
              ...
              <div className="flex flex-wrap items-center gap-8 text-white/80 border-t border-white/10 pt-10 mt-4 font-sans">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs text-white/40 uppercase tracking-widest mb-0.5">Author</span>
                    <span className="text-sm font-bold tracking-wide text-white">{post.authorName}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-6 ml-auto print:hidden">
                  <button onClick={shareBlog} className="hover:text-emerald-400 transition-all hover:scale-110 cursor-pointer"><Share2 size={24} /></button>
                  <button onClick={toggleBookmark} className={`transition-all hover:scale-110 cursor-pointer ${isBookmarked ? 'text-emerald-400 fill-emerald-400' : 'hover:text-emerald-400 text-white'}`}>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="prose prose-emerald lg:prose-xl max-w-none">
            <div className="whitespace-pre-line">
              <ReactMarkdown
                components={{
                  p: ({node, ...props}) => <p className="text-stone-600 text-xl font-serif leading-[1.8] mt-[12px] mb-[12px]" {...props} />,
                  h2: ({node, ...props}) => <h2 className="font-serif text-3xl font-bold text-primary mt-12 mb-6" {...props} />,
                  h3: ({node, ...props}) => <h3 className="font-serif text-2xl font-bold text-primary mt-8 mb-4" {...props} />,
                  ul: ({node, ...props}) => <ul className="list-disc pl-6 space-y-4 my-8 text-stone-600" {...props} />,
          ...
          {post.syndicationLinks && post.syndicationLinks.length > 0 && (
            <div className="mt-12 pt-8 border-t border-stone-100 print:hidden">
              <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#b58e3e] mb-8">External Features</h4>
              <div className="flex flex-wrap gap-4">
                {post.syndicationLinks.map(link => (
                  ...
                  >
                    ...
                    <ExternalLink size={16} className="text-stone-300 group-hover:text-emerald-600 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                  </a>
                ))}
              </div>
            </div>
          ...
          <div className="mt-24 pt-0 border-t border-stone-100 flex flex-col md:flex-row items-center justify-between gap-8 print:hidden">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Story By</p>
                <p className="font-serif text-lg font-bold text-primary">{post.authorName}</p>
              </div>
            </div>
