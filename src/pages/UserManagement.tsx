...
export function UserManagement() {
  ...
  const createUserMutation = useMutation({
    mutationFn: (newMember: { name: string; email: string; role: UserRole }) =>
      api.createUser(newMember),
    ...
    onSuccess: (newUser) => {
      ...
      setNotification({
        type: "success",
        message: `Invitation sent to ${newUserData.email}!`,
      });
  ...
  const deleteUserMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      setNotification({
        ...
        message:
          error.message ||
          "The server encountered an error while trying to remove this member.",
      });
      ...
      <div className="flex-1 flex items-center justify-center p-12 text-center">
        <div className="max-w-md">
          ...
          <p className="text-stone-500">
            Only administrators can access the staff registry. Please consult
            with leadership if you believe this is an error.
          </p>
        </div>
      </div>
    );
  ...
  const removeUser = (id: string) => {
    if (id === user?.id) {
      setNotification({
        type: "error",
        message:
          "You cannot remove your own administrative access while logged in.",
      });
      return;
    }

    if (
      confirm("Are you sure you want to remove this member from the kitchen?")
    ) {
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
          <div>
            ...
            <p className="text-secondary text-lg">
              Manage the culinary minds behind Nourish Kitchen.
            </p>
          </div>
          <button
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-stone-50/50">
                    <th className="px-8 py-5 text-[10px] font-bold text-stone-400 uppercase tracking-[0.2em]">
                      Member
                    </th>
                    <th className="px-8 py-5 text-[10px] font-bold text-stone-400 uppercase tracking-[0.2em]">
                      Role
                  ...
                  {filteredUsers.map((item: User) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${item.status === "active" ? "bg-emerald-500" : "bg-amber-500 animate-pulse"}`}
                          />
                        ...
                        {item.id !== "1" && (
                          <button
                            ...
                            disabled={
                              ...
                              deleteUserMutation.variables === item.id
                            }
                ...
                <form onSubmit={handleAddUser} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Mail
                        size={14}
                        className="absolute left-0 bottom-3 text-stone-300"
                      />
                      <input
                        required
                        type="email"
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <button
                        type="button"
                        onClick={() =>
                          setNewUserData({ ...newUserData, role: "CHEF" })
                        }
                        className={`p-4 rounded-2xl border text-center transition-all ${
                          newUserData.role === "CHEF"
                            ? "border-primary bg-[#e7f0e4] text-primary"
