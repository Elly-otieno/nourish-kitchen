...
import { Star, Clock, ChefHat, Heart, ArrowRight, TrendingUp, Sparkles, Coffee, Flame, Utensils, CheckCircle2, Trash2, AlertCircle, Loader2 } from 'lucide-react';
...
export function Landing() {
  ...
  const handlesBlogDelete = async (id: string) => {
    if (confirm('Move this journal entry to archives?')) {
      try {
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-30 w-full flex flex-col items-center">
          ...
          >
            <div className="flex items-center justify-center gap-3 mb-8">
              ...
              <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.5em] text-white/80">Est. 2025 · From our kitchen to yours</span>
              <div className="h-px w-8 bg-white/40" />
            </div>

            <h1 className="font-serif text-6xl md:text-8xl lg:text-[110px] text-white leading-[0.95] tracking-tight mb-8">
              Savor the <br />
              <span className="italic font-light opacity-90 block mt-2">moment.</span>
            </h1>

            <p className="text-lg md:text-xl text-white/80 font-sans max-w-xl mx-auto mb-12 leading-relaxed font-light tracking-wide">
              Welcome to our home kitchen. Discover wholesome, seasonal recipes and the simple joy of cooking for the people you love.
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
              ...
              >
                ...
                <ArrowRight className="absolute right-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:translate-x-2" size={16} />
              </Link>
          ...
          >
            <span className="text-[9px] font-black uppercase tracking-[0.4em] text-white/30 rotate-0">Scroll</span>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            <div className="max-w-2xl">
              <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-800/40 mb-4">The Selection</h2>
              <h3 className="font-serif text-5xl md:text-6xl text-emerald-950 leading-[1.1] tracking-tight">
                Curated <span className="italic font-light">Heritage</span> <br /> Recipes for the Modern Table
              </h3>
            </div>
            <Link to="/recipes" className="group flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] text-emerald-900 border-b border-emerald-900/10 pb-2 hover:border-emerald-900 transition-all">
            ...
            {featuredRecipes.map((recipe, idx) => (
              ...
              >
                <Link to={`/recipes/${recipe.id}`} className="block">
                  <div className="aspect-[4/5] overflow-hidden rounded-[2.5rem] mb-8 relative">
                    ...
                    <div className="absolute top-6 right-6 flex flex-col gap-3">
                      <div className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-full flex items-center gap-2 shadow-xl border border-white/20">
                        ...
                        <span className="text-[9px] font-black tracking-widest text-emerald-950">{recipe.rating}</span>
                      </div>
                      {(user?.role === 'ADMIN' || user?.role === 'CHEF' || user?.id === recipe.authorId) && (
                        <button 
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                  ...
                  <div>
                    <span className="text-[9px] font-black uppercase tracking-[0.4em] text-emerald-600 mb-3 block">{recipe.categories[0]}</span>
                    <h4 className="font-serif text-2xl text-emerald-950 group-hover:text-emerald-800 transition-colors mb-4">{recipe.title}</h4>
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        <Clock size={12} className="text-stone-300" />
                        <span className="text-[9px] font-bold text-stone-400 uppercase tracking-widest">{recipe.prepTime}</span>
                      </div>
                      <div className="w-1 h-1 rounded-full bg-stone-200" />
                      <div className="flex items-center gap-2">
                        <Utensils size={12} className="text-stone-300" />
                        <span className="text-[9px] font-bold text-stone-400 uppercase tracking-widest">{recipe.cuisine}</span>
                      </div>
                    </div>
                  </div>
                </Link>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            {[
              { label: 'Morning', icon: Coffee, image: 'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&q=80&w=400', path: '/recipes?cat=breakfast' },
              { label: 'Mindful', icon: Utensils, image: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?auto=format&fit=crop&q=80&w=400', path: '/recipes?cat=vegetarian' },
              { label: 'Gather', icon: Utensils, image: 'https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&q=80&w=400', path: '/recipes?cat=dinner' },
              { label: 'Sweet', icon: Flame, image: 'https://images.unsplash.com/photo-1488477181946-6428a0291777?auto=format&fit=crop&q=80&w=400', path: '/recipes?cat=desserts' },
              { label: 'Quick', icon: Clock, image: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&q=80&w=400', path: '/recipes?cat=quick' },
            ].map((cat, idx) => (
              <Link 
                key={cat.label} 
                to={cat.path}
              ...
              >
                <div className="absolute inset-0">
                  <img src={cat.image} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" alt={cat.label} />
                ...
                <div className="absolute inset-x-0 bottom-8 px-6 flex flex-col items-center">
                   ...
                   <span className="text-[10px] font-black tracking-[0.4em] text-white uppercase">{cat.label}</span>
                </div>
              </Link>
            ))}
          </div>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="text-center mb-24 max-w-3xl mx-auto">
            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-emerald-800/40 mb-6 block">Our Latest Creations</span>
            <h2 className="font-serif text-5xl md:text-6xl text-emerald-950 mb-8 leading-tight">Fresh from the Kitchen</h2>
            <div className="w-12 h-px bg-emerald-900/20 mx-auto mb-8" />
            <p className="text-stone-500 text-lg leading-relaxed font-light italic">"Stories of flavor, family, and the simple joy of a well-cooked meal."</p>
          </div>
              ...
              {latestFinds.map((item, idx) => (
                <motion.div 
                  ...
                  className={`group relative cursor-pointer overflow-hidden rounded-[2.5rem] bg-stone-100 ${
                    idx % 3 === 0 ? 'lg:row-span-2 aspect-[3/4]' : 'aspect-square'
                  }`}
                >
                  <Link to={`/recipes/${item.id}`} className="block h-full">
                    ...
                    {(user?.role === 'ADMIN' || user?.role === 'CHEF' || user?.id === item.authorId) && (
                      <button 
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-8 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                      <div className="bg-white/90 backdrop-blur-md p-6 rounded-[1.5rem] shadow-2xl border border-white/20">
                        <span className="text-[8px] font-black uppercase tracking-[0.3em] text-emerald-600 mb-2 block">{item.categories[0]}</span>
                        <h4 className="text-emerald-950 font-serif text-lg font-bold leading-tight">{item.title}</h4>
                      </div>
                    </div>
                  </Link>
                </motion.div>
          ...
          {hasMoreRecipes && (
            <div className="mt-20 text-center">
              ...
              >
                Expand the Collection <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
          )}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-20">
            <div className="max-w-xl">
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-emerald-900/40 mb-6 block">Our Stories</span>
              <h2 className="font-serif text-5xl md:text-6xl text-emerald-950 leading-tight">Letters from <br /><span className="italic font-light">the Heart of the Home</span></h2>
            </div>
            <Link to="/blog/all" className="group flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.3em] text-emerald-900 border-b border-emerald-900/10 pb-3 hover:border-emerald-900 transition-all">
                ...
                <Link to={`/blog/all/${post.id}`} className="block">
                  <div className="aspect-[4/5] overflow-hidden rounded-[2.5rem] mb-10 relative shadow-2xl shadow-stone-200">
                    ...
                    <div className="absolute top-6 left-6 flex items-center gap-3">
                      ...
                      {(user?.role === 'ADMIN' || user?.role === 'CHEF' || user?.id === post.authorId) && (
                        <button 
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                  ...
                  <div className="px-2">
                    <span className="text-[9px] font-black uppercase tracking-[0.5em] text-stone-300 mb-3 block">{post.date}</span>
                    <h3 className="font-serif text-3xl text-emerald-950 mb-6 group-hover:text-emerald-800 transition-colors leading-[1.2]">{post.title}</h3>
                    <p className="text-stone-500 text-base leading-relaxed line-clamp-2 font-light italic">"{post.excerpt}"</p>
                    <div className="mt-8 flex items-center gap-3">
                       ...
                       <span className="text-[8px] font-black uppercase tracking-[0.3em] text-emerald-900/40 group-hover:text-emerald-900 transition-colors">Read Memoir</span>
                    </div>
                  </div>
                </Link>
              </motion.div>
        ...
        <div className="max-w-7xl mx-auto bg-emerald-50 rounded-[32px] p-12 md:p-24 overflow-hidden relative">
          <div className="relative z-10 max-w-2xl ">
            <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold text-emerald-950 mb-8 leading-tight">
              ...
              <span className="italic font-normal serif-font">Straight to your Inbox.</span>
            </h2>
            <p className="text-lg text-stone-600 mb-10 leading-relaxed font-medium">
              Join several home chefs receiving weekly dinner inspiration, stories from the garden, and kitchen tips.
            </p>
            ...
            {subscribed ? (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-emerald-50 text-sm font-medium">Check your inbox for your first kitchen update.</p>
                </div>
              </motion.div>
            ) : (
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-4">
                ...
                />
                <button type="submit" className="bg-emerald-950 text-white px-10 py-5 rounded-lg font-bold text-[11px] uppercase tracking-widest hover:bg-emerald-900 transition-colors shadow-lg" id="subscribe-button">
                  Join Now
                </button>
              </form>
            )}
            
            <p className="mt-6 text-[10px] font-bold text-stone-400 uppercase tracking-widest">No spam. Only heritage. Unsubscribe anytime.</p>
          </div>
        </div>
      </motion.section>
