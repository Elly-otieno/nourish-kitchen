...
export function RecipeDetail() {
  ...
  // ==========================================
  const { data: recipe = null, isLoading: loading } = useQuery<Recipe | null>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        return await api.getRecipe(String(recipeId));
      } catch (error) {
  ...
  const deleteRecipeMutation = useMutation({
    mutationFn: (idToDelete: number) => api.deleteRecipe(String(idToDelete)),
    onSuccess: () => {
      setNotification({
        type: "success",
  ...
  const toggleLikeMutation = useMutation({
    mutationFn: async () => {
      ...
      return await api.toggleLikeRecipe(String(recipe.id));
    },
    onSuccess: (updatedRecipe) => {
      ...
      setIsLiked(updatedRecipe.liked);
  ...
  useEffect(() => {
    if (recipe && user) {
      setIsLiked(recipe.liked_by?.includes(Number(user.id)) || false);
    }
  }, [recipe, user]);
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 sm:p-0">
            ...
            >
              ...
              <p className="text-stone-500 text-sm mb-8 leading-relaxed">
                This recipe will be removed from the public library but
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-12 pt-8 pb-24">
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 mb-10 pb-10 border-b border-stone-100">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full overflow-hidden border border-stone-200">
                <img
                  src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${recipe.author_name}`}
              ...
              <div className="text-left">
                ...
                <Link
                  to={`/users/${recipe.author}`}
                  className="font-serif text-base font-bold text-primary hover:text-emerald-700 transition-colors"
                >
                  {recipe.author_name}
                </Link>
              </div>
        ...
        <section className="max-w-3xl mx-auto mb-24 md:mb-32">
          <div className="flex items-center justify-center gap-3 mb-12">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#b58e3e]">
              The Story
            </p>
            <span className="h-px w-12 bg-stone-200" />
          </div>
            ...
            <div className="font-sans text-stone-700 leading-[1.8] space-y-8 text-xl whitespace-pre-line">
              <ReactMarkdown
                components={{
                  h3: ({ node, ...props }) => (
                    <h3
                      className="font-serif text-3xl md:text-4xl font-bold text-emerald-950 mt-0 mb-0"
                      {...props}
                    />
                  ),
                  p: ({ node, ...props }) => (
                    <p className="mt-0 mb-0" {...props} />
                  ),
                  strong: ({ node, ...props }) => (
                    <strong
                      className="font-bold text-stone-900 border-b-2 border-primary/20 pb-0.5"
                      {...props}
                    />
                  ),
                  ul: ({ node, ...props }) => (
                    <ul className="list-disc pl-6 space-y-4 my-8" {...props} />
                  ),
                  li: ({ node, ...props }) => (
                    <li className="text-lg text-stone-600" {...props} />
                  ),
                }}
              >
                {recipe.story}
        ...
        {/* Recipe Card Section */}
        <div
          className="bg-[#fcfbf9] rounded-[3rem] p-8 md:p-16 border border-stone-100 shadow-sm scroll-mt-24"
          id="recipe-card"
        ...
        >
          ...
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-20">
            ...
            <aside className="lg:col-span-5">
              <div className="flex justify-between items-end mb-10">
                ...
                <div className="flex flex-col items-end print:hidden">
                  <span className="text-[10px] font-bold text-stone-400 mb-2 uppercase tracking-widest">
                    Servings
                  </span>
              ...
              <div className="space-y-10">
                {Array.from(
                  new Set(recipe.ingredients.map((i) => i.category)),
                ).map((cat, idx) => (
                  <motion.div
                    key={cat}
                      ...
                      {recipe.ingredients
                        ...
                        .map((item) => (
                          <li
                            ...
                            onClick={() => toggleIngredient(String(item.id))}
                          >
                            <div
                              className={`mt-1.5 h-4 w-4 rounded border flex items-center justify-center transition-colors shrink-0 ${
                            ...
                            <span
                              className={`text-base md:text-lg font-sans ${checkedItems[item.id] ? "text-stone-400 line-through" : "text-stone-700 font-medium"}`}
                            >
                ...
                <motion.div className="mt-12 p-8 bg-emerald-50/50 border-l-4 border-primary rounded-r-3xl relative">
            ...
            <article className="lg:col-span-7">
              <h3 className="font-serif text-3xl font-semibold mb-10 text-primary">
                Instructions
              </h3>
              ...
              >
                ...
                <div className="relative z-10 px-8 md:px-16 py-12 md:py-20 flex flex-col md:flex-row items-center justify-between gap-12 border border-white/10 rounded-[2.5rem]">
                  <div className="max-w-xl text-center md:text-left">
                    <span className="text-[10px] font-black uppercase tracking-[0.6em] text-emerald-400/80 mb-6 block">
                      Master the Technique
                    </span>
                    <h3 className="font-serif text-3xl md:text-4xl text-white mb-6 leading-tight">
                      Witness the{" "}
                      <span className="italic font-light">Heritage</span> in
                      motion.
                    </h3>
                    <p className="text-emerald-50/50 text-base md:text-lg leading-relaxed font-light italic">
                      "Observe the traditional methods, the rhythm of the blade,
                      and the transformation of ingredients into a legacy of
                      flavor."
                    </p>
                  </div>
        ...
        <section className="mt-24 border-t border-stone-100 max-w-4xl mx-auto print:hidden">
          <QuestionsSection entityId={String(recipe.id)} />
          <CommentsAndReviewsSection entityId={String(recipe.id)} />
