...
import { MessageSquare, Send, CheckCircle, Trash2, Reply, ShieldAlert } from 'lucide-react';
import { api } from '../services/api';
import { Comment } from '../types';
import { useAuth } from '../contexts/AuthContext';

interface CommentsSectionProps {
  entityType: 'recipe' | 'blog_post';
  entityId: number;
}
...
export function CommentsSection({ entityType, entityId }: CommentsSectionProps) {
  ...
  const loadComments = async () => {
    try {
      ...
      // Filter cleanly using your interface's specific entity fields
      const relevant = allComments.filter(c => {
        if (entityType === 'recipe') return c.recipe === entityId;
        if (entityType === 'blog_post') return c.blog_post === entityId;
        return false;
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Build a submission payload matching your exact database interface properties
      const commentData: Partial<Comment> = {
        recipe: entityType === 'recipe' ? entityId : null,
        blog_post: entityType === 'blog_post' ? entityId : null,
        content: newComment,
        parent: replyTo ? replyTo.id : null,
        type: entityType.toUpperCase(),
        is_anonymous: isAnonymous,
        user: isAnonymous ? null : (user?.id ? Number(user.id) : null),
        user_name: isAnonymous ? 'Anonymous Explorer' : (user?.username || 'Guest User'),
        user_avatar: isAnonymous 
          ? 'https://api.dicebear.com/7.x/shapes/svg?seed=anon' 
          : (user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.username || 'guest'}`),
  ...
  const handleApprove = async (id: number) => {
    try {
      await api.approveComment(String(id));
      loadComments();
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    try {
      await api.deleteComment(String(id));
      loadComments();
    } catch (error) {
  ...
  // Comments without a parent are core root comment blocks
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Form */}
      <form onSubmit={handleSubmit} className="bg-stone-50 p-8 rounded-4xl border border-stone-100">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-10 h-10 rounded-full overflow-hidden bg-white border border-stone-200">
            <img 
              src={isAnonymous ? 'https://api.dicebear.com/7.x/shapes/svg?seed=anon' : (user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.username || 'guest'}`)} 
              alt="avatar" 
            />
          </div>
          <div className="flex-1">
            <p className="text-xs font-bold text-stone-900">
              {replyTo ? `Replying to ${replyTo.user_name}` : (isAnonymous ? 'Sharing Anonymously' : (user?.username || 'Joining the conversation'))}
            </p>
            {replyTo && (
              <button 
                type="button"
                onClick={() => setReplyTo(null)} 
                className="text-[10px] text-primary hover:underline uppercase tracking-widest font-black mt-0.5 block"
              >
                Cancel Reply
              </button>
            )}
        ...
        <textarea 
          ...
          className="w-full bg-white border border-stone-100 rounded-2xl p-6 font-sans text-sm md:text-base focus:ring-2 focus:ring-primary/10 transition-all min-h-30 mb-6 outline-none"
        />
...
const CommentItem: React.FC<CommentItemProps> = ({ comment, replies, onReply, onApprove, onDelete, isStaff, isAdmin }) => {
  return (
    <div className="group">
      <div className={`p-8 rounded-4xl border transition-all ${comment.is_approved ? 'bg-white border-stone-100' : 'bg-amber-50/50 border-amber-100'}`}>
        {!comment.is_approved && (
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            <div className="flex justify-between items-start mb-2">
              <h4 className="font-serif text-lg font-bold text-stone-900">{comment.user_name}</h4>
              <span className="text-[10px] font-bold text-stone-300 uppercase tracking-widest">{comment.created_at}</span>
            </div>
            ...
            <div key={reply.id} className="p-6 bg-stone-50 rounded-2xl flex gap-4">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-1">
                  <h5 className="text-sm font-bold text-stone-900">{reply.user_name}</h5>
                  <span className="text-[9px] font-bold text-stone-300 tracking-widest">{reply.created_at}</span>
                </div>
